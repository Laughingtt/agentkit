...
## 一·甲、初学者可懂性原则（最高优先级，2026-09 用户约定）

> **核心判断：如果读者读完后不能用一句话转述本章要点，本章就是失败的——无论它有多"完整"或"准确"。**

本书目标读者**会写代码但没有 AI/ML 背景**。所有内容必须按"无 AI 背景的程序员"为假想读者写作。这一条**凌驾于**"完整性"、"覆盖度"、"技术深度"、"篇幅控制"等其他原则之上——遇到冲突时，永远优先可懂性。

### 写作铁律（每条都必须在每节、每段、每图时复检）

1. **认知负荷控制** —— 一个段落同时塞给读者的新概念 ≤ 3 个。如果一节要介绍 4 个以上新概念，必须**显式分小节**，每个小节只聚焦 1 个核心概念；其余概念推到"Part 2 见"或"本章末参考"的 brief 提示里。
2. **直觉 → 类比 → 视觉 → 公式** —— 任何公式或抽象概念出现前，必须**已经**完成前三步：
   - **直觉**（plain language）：用一句话说出"它在做什么、为什么这样做"
   - **类比**（analogy）：一个生活中的对应物（图书馆员、高亮笔、餐厅点菜、流水线……）
   - **视觉**（visual）：一个 mermaid 图 / ASCII 图 / 表格
   - **公式**（formula）：最后才用 LaTeX 精确定义
   - 这四步的顺序**不可颠倒**。读者在第 1 步还糊涂，到了第 4 步会更糊涂。
3. **不堆砌术语首字母缩写** —— 同一段里出现的英文缩写 ≤ 3 个。CAI/RLAIF/PRM/ORPO/SimPO/KTO 这类小节最多保留 1-2 个展开讲，其余合并到"其他变体一句话点名"。
4. **不炫技式罗列** —— 列出"MoE、RMSNorm、SwiGLU、GQA"四种现代变体看似丰富，但读者一个都记不住。**采用"一招鲜 + 一句话点名"模式**：主推一种作为范例深入讲，其余只在表格里一句话提及 + Part 2 预告。
5. **worked-example effect** —— 抽象概念给出前，先给一个**具体可触摸的小例子**（一个句子、一个矩阵、一个 token 序列）。抽象符号只对已经理解具体例子的读者才有意义。
6. **Split-Attention 防护** —— mermaid 图 / 表格**紧贴**它解释的段落，不要隔三段。读者回头看图时要一眼能找到对应文字。
7. **每章最多 1-2 个 mermaid 架构图 + 多张局部小图** —— 章节开头一张"全图导览"定锚，后续小节配 1-2 张局部流程图、对比表、ASCII 示意图。不要等所有内容讲完才出一张大图。
8. **冗余覆盖原则（contextual repetition）** —— 关键概念在第一次出现时讲透，但**简短回顾**在后续小节首段再次出现。重复不是冗余——是给读者搭阶梯。
...
|---|---|
| 一节 5+ 个新概念 | 读者一个都记不住 |
| 公式先于直觉出现 | 公式被当成"必填公式"跳读 |
| 5+ 个英文缩写同段出现 | 阅读时全程卡顿 |
| 表格只有比较、没有例子 | 表格读起来像背字典 |
| 一节里讲完整数学推导 | 偏离"直觉优先"原则 |
| 一章 1 张图讲完整章内容 | 读者抓不住局部细节 |
| 缺乏"为什么这样做"的动机 | 概念沦为术语空壳 |
...
- [ ] 这一节引入了几个新概念？（目标 ≤ 3）
- [ ] 第一个公式出现前，读者是否已有直觉 + 类比 + 视觉？
- [ ] 缩写数量 ≤ 3 个？
- [ ] mermaid 图 / 表格 紧邻它解释的段落？
- [ ] 读者能否用一句话转述本节要点？
- [ ] 如果我去掉所有公式，读者还能跟着读完吗？（这检验了直觉铺垫是否到位）
...
### 验证的批量执行（2026-09 用户约定）

为加快写作节奏，**单章写作阶段只做最小自查**（仅核查自己刚写的内容是否有明显事实错误、链接是否格式正确）。**完整的写后审查统一在最后一个 Task（Task 6 整合与终审）中执行**，包括：

- 所有 URL 用 WebFetch 抽样验证
- 所有 Python 代码统一运行
- 字数统计（wc -m）
- 内部一致性、范围、风格的全局检查

Task 6 之前的所有 Task 不需要做完整审查，只需保证写作基本准确 + commit + 写 report。