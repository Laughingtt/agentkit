...
### 表达下限与审查顺序（2026-09 用户约定）

> **"再准确的文章，没有读者又有什么意义。"**

1. **表达下限** —— 解释任何专有名词时，默认读者只具备**小学/初中知识**，不默认他懂任何 AI/ML 词汇。术语**不能裸用**：任何术语首次出现前，必须已完成"**白话一句话 + 生活类比 + 具体小例子**"三件套铺垫，之后才允许出现术语名。写法上禁止单句堆术语、禁止百科词条式定义句；通俗化牺牲掉的精确表述，放章末参考与 Part 终审兜底。
2. **审查顺序** —— 单章完成后的审查**只审易读性与理解性**（能否一句话转述、术语是否裸用、类比是否到位、有没有吸引力）；**所有事实准确性核查统一推迟到该 Part 的整合终审（Task 6）**。单章阶段不为事实核查消耗时间——力气全部花在"读者读不读得懂"上。
...
- 章末 **本章参考** 分三类：
  1. 必读论文（确无论文的协议/工程章可写作「必读」，如 Part 2 Ch9）
  2. 推荐博客 / 教程
  3. 视频 / 课程
...
### 写后审查（在 Part 终审 Task 6 执行；单章阶段只审易读性与理解性）
...
**单章审查只审易读性与理解性**——一·甲检查清单、表达下限三件套（白话/类比/例子）、能否一句话转述；**事实准确性、链接、代码运行一律不查**。**所有准确性核查统一在最后一个 Task（Task 6 整合与终审）中执行**，包括：
...
理由（用户原话）："再准确的文章，没有读者又有什么意义。"Task 6 之前的 Task 只需：读得懂 + commit + 写 report。