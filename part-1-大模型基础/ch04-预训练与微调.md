...
**第一步：来源聚合。** 主流 LLM 的预训练数据通常由多个来源拼成：CommonCrawl 网页抓取、GitHub 代码、Wikipedia、Books、StackExchange 问答、arXiv 论文等。

**第二步：清洗。** CommonCrawl 体量最大但也最杂。典型清洗步骤：去 HTML 标签；用 fastText 这类模型做语言识别（LID）；启发式过滤（去掉重复段落、过短/过长文档）；用轻量分类器判断文档质量打分。

**第三步：去重。** 网页里大量"模板化"的页面（产品列表、SEO 农场）整段重复。如果不去重，模型会过度记忆这些模式，损害泛化能力。常用 **MinHash** 或 **exact hash** 在文档级和段落级都做一遍去重。

**第四步：配比（mixture）。** 清洗完后还要决定**每个来源各取多少**——这就是数据配比。比如 LLaMA-3 的数据配比大致是：英语网页 ~50%、代码 ~15%、多语言 ~8%、高质量问答 ~5%、书籍 ~5%。配比对模型"擅长什么"影响巨大——代码占比高，模型编程就强；中文占比低，中文就弱。

整套流水线跑下来，原始语料常常被砍掉 90% 以上。**质量比数量更重要**，是现代 LLM 数据工程的共识。
...
- **WordPiece**：和 BPE 思路很像，但合并准则是"似然增益最大化"而不是"频率最大化"。BERT 用 WordPiece。
...
把模型训起来，靠一张 GPU 是远远不够的。一个 70B 参数的 LLM，光参数就要 ~140 GB 显存（fp16），一张 H100（80GB）连模型都装不下。所以**分布式训练是必然**，业界用三种并行方式：

**数据并行（DP）**：把数据切碎，每个 GPU 拿到完整的模型副本 + 一份数据切片，各自算梯度，然后**同步梯度**（AllReduce）。

**张量并行（TP）**：把单个矩阵乘法**按维度切开**到多张卡上。比如把一个 $[d \times d]$ 的权重切成两块 $[d \times d/2]$，分别放到两张卡上算，最后 concat。Megatron-LM 把这套方法定型成工业标准。

**流水线并行（PP）**：把模型的**层**切成几段，每张卡负责一段。卡 0 算 layer 1~16，卡 1 算 17~32，……数据像流水线流过去。

实际工业实践中三种并行**经常组合使用**：8 路 TP + 16 路 PP + 64 路 DP = 8192 张卡，正是 GPT-4 训练集群的典型规模。配套还有 **ZeRO**（优化器状态切分）、**FSDP**、**DeepSpeed** 等工具链——这些是 Part 2 才会深入的话题。
...
最早系统研究这个问题的是 OpenAI 的 Kaplan 等人（2020，参见本章参考）。他们发现：当固定算力时，模型性能与参数量的对数呈幂律关系——**模型每变大 10 倍，损失下降一个稳定比例**。

但 Kaplan 的结论被 DeepMind 的 **Chinchilla**（Hoffmann et al., 2022）修正了。Chinchilla 用同一套算力训练了一系列不同 $N$ 和 $D$ 的组合，发现：**最优的模型大小和数据量应该按比例同步增长**。给定一份算力预算（比如 1e24 FLOPs），最优配比大约是 **每 1 个参数配 20 个训练 token**。Chinchilla-70B 只用 70B 参数 + 1.4T token，就击败了 GPT-3 的 175B 参数 + 300B token——证明了"模型不是越大越好，数据要和模型匹配"。

Chinchilla 的影响深远。今天几乎所有 LLM 训练都会参考"1 参数配 20 token"这条经验线。Chinchilla 的核心思想——**算力要在模型和数据之间平衡分配**——至今是 LLM 训练的第一性原则。
...
- **合成数据**：用规则、模板、强模型生成针对特定技能的样本（如数学题、代码题）。
...
训练时：**冻结原始 $W$，只训练 $A$ 和 $B$**。新增参数只有 $r \times (d + k)$ 个——当 $r=8, d=k=4096$ 时，新增参数不到 $W$ 的 $0.4\%$。推理时把 $B \cdot A$ 合并回 $W$，**不增加任何额外延迟**。

工程细节：LoRA 通常只加在 attention 的 $W^Q, W^V$ 上；$A$ 用随机高斯，$B$ 用零——保证训练开始时 $\Delta W = 0$；不同任务训练不同的 $(A, B)$，共享同一个 base $W$。

**QLoRA（Dettmers et al., 2023）** 把 LoRA 推到极限。它先把 base 模型**量化到 4-bit（NF4 数据类型）**，显存占用砍到原来的 1/4；然后在量化的 base 上做 LoRA 微调。QLoRA 让一张 24GB 显存的消费级 GPU（如 RTX 4090）就能微调 65B 参数的模型——**让个人开发者也能负担得起 LLM 微调**。
...
- **与预训练数据混训**：在 SFT 数据里掺入 10%~30% 的预训练语料，让模型在"学新"的同时"复习旧"。LLaMA-2-Chat 训练流程里就明确用了这个技巧。
- **正则化**：EWC（Elastic Weight Consolidation）给重要参数加约束，阻止它们偏离原值太远。
...
2. **中文按"字/词"切**：`"核心架构"` 被切成 `核心` + `架构` 两个 token——SentencePiece 在中文上更像"按词切"；而单字如 `"是"` `"的"` `"。"` 各占一个 token。
3. **可逆性**：最后那个 `assert` 验证 `decoded == text`，证明 **tokenizer 是无损的**——编码再解码，可以精确还原原文。这条性质非常关键：模型看到的输入和生成的输出都是 token 流。
...
（LLaMA-3 数据配比与训练流程的官方权威解读：阅读 Meta 的 "The Llama 3 Herd of Models" 技术报告原文，特别是 Data、Pre-Training、Post-Training 三章，里面给出了 8B/70B/405B 三档模型完整的数据配比与训练流程。）
