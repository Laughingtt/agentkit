...
```mermaid
graph LR
    A[原文] --> B[切分] --> C[转 id] --> D[解码]
```

主流的 tokenizer 几乎都是**子词**方案，常用三种：

- **BPE（Byte Pair Encoding）**：从字符集开始，反复把出现频率最高的相邻字符合并成新符号。GPT-2/3/4、Qwen-2.5 都用 BPE（Qwen2.5 用的是 tiktoken 风格）。
- **WordPiece**：和 BPE 思路很像，但合并准则是"似然增益最大化"而非频率。BERT 用 WordPiece。
- **SentencePiece**：把文本当作 Unicode 字符流处理（**不依赖空格**），天然支持中日韩等无空格语言。最常用算法是 **Unigram**（基于概率剪枝）。LLaMA-2、DeepSeek 系列用 SentencePiece。

| 方案 | 切分依据 | 是否依赖空格 | 代表模型 |
|---|---|---|---|
| BPE | 频率合并 | 依赖（预分词） | GPT-2/3/4、Qwen-2.5 |
| WordPiece | 似然增益 | 依赖（预分词） | BERT |
| SentencePiece (Unigram) | 概率剪枝 | 不依赖 | LLaMA-2 / DeepSeek |

**中文场景特别提醒**：英文词与词之间有空格，预分词容易；中文没有空格，预分词本身就难。SentencePiece / tiktoken-style BPE 把整段文本当成 Unicode 流处理，是目前处理中文最稳的方案。这也是 Qwen、DeepSeek 等中文 LLM 都走这条路的原因。
...
**简单记法**：DP 切样本、TP 切矩阵、PP 切层。GPT-4 训练集群据估算动用了**上万张 GPU**（Meta 公布的 24k H100 集群是同级别参照）。配套还有 **ZeRO**、**FSDP**、**DeepSpeed** 等工具链——这些是 Part 2 才会深入的话题。
...
```mermaid
graph LR
    X[输入 x] --> W[W 4096×4096<br/>冻住]
    W --> OUT[输出]
    X --> M[B 4096×8 × A 8×4096<br/>训练中]
    M --> SUM[与主路求和]
```
...
训练时**冻结 $W$，只训练 $A$ 和 $B$**。$\Delta W \approx B \cdot A$，其中 $A$ 用随机高斯、$B$ 用零初始化——保证训练开始时 $\Delta W = 0$。推理时把 $B \cdot A$ 合并回 $W$，**不增加任何额外延迟**。LoRA 通常只加在 attention 的 $W^Q, W^V$ 上。

**QLoRA（Dettmers et al., 2023）** 把 LoRA 推到极限：先把 base **量化到 4-bit（NF4 数据类型）**，显存砍到原来的 1/4，再在量化的 base 上做 LoRA。QLoRA 让一张 24GB 显存的消费级 GPU（如 RTX 4090）就能微调 65B 参数的模型——**让个人开发者也能负担得起 LLM 微调**。
...
- **与预训练数据混训**：在 SFT 数据里掺 10%~30% 预训练语料，让模型"学新"的同时"复习旧"。LLaMA-2-Chat 训练流程里就用了这个。
...
（LLaMA-3 数据配比与训练流程的官方权威解读：阅读 Meta 的 "The Llama 3 Herd of Models" 技术报告原文，特别是 Data、Pre-Training、Post-Training 三章。）
