...
**AI（人工智能，Artificial Intelligence）** 是一个目标——让机器表现出通常需要人类智慧才能完成的行为，比如识别猫狗、理解一段话、下棋、开车。这个目标从 1956 年的达特茅斯会议被正式提出，70 年来尝试过很多不同路线。

**ML（机器学习，Machine Learning）** 是实现 AI 的一类方法。传统编程是"人写规则、机器执行"；机器学习反过来——人准备数据，机器从数据里自己学规则。它是 AI 的一个子集。

**DL（深度学习，Deep Learning）** 又是 ML 的一个子集，特指用"多层神经网络"做函数逼近的方法。多层意味着模型可以有上亿个参数，从而拟合非常复杂的模式。
...
再举一个识别猫的例子：传统规则派会写"有尖耳朵、有胡须、四条腿→猫"，很脆弱；机器学习派会喂 10 万张 (图片, "猫/不是猫") 样本让算法自己找规律；深度学习派会用上百层神经网络，让前几层学边缘、中间层学眼睛、最后几层组合出"猫"概念——特征不用人设计，模型自己学。
...
**符号主义（Symbolism / Good Old-Fashioned AI）** 认为智能可以用"符号 + 规则"来表达。知识就是事实（如"苏格拉底是人"），推理就是逻辑规则的演绎（如"人都会死，苏格拉底是人，所以苏格拉底会死"）。代表成果有 1980 年代的专家系统（如 MYCIN 医疗诊断）、Cyc 常识知识库、知识图谱，以及 Prolog 这类逻辑编程语言。优点是可解释——每一步推理都能追溯；缺点是知识获取成本极高、面对模糊或噪声数据非常脆弱。
...
**统计学习（Statistical Learning）** 用概率和统计工具来建模，主张"知识 = 数据背后的概率分布"。代表方法有支持向量机（Support Vector Machine, SVM）、贝叶斯网络、隐马尔可夫模型（Hidden Markov Model, HMM）、条件随机场（Conditional Random Field, CRF）。优点是数学基础扎实、泛化理论清晰；缺点是特征往往需要人工设计，在图像、语音这种原始信号上表现吃力。
...
机器学习内部又可以根据"数据里有没有标注、学习信号怎么来"分成三大学习范式。下表给出一句话定义和典型任务，读者可以快速对照记忆。
...
**无监督学习**不需要人给答案，让模型自己找结构。K-Means 能把 100 万用户按消费习惯自动分成 5 类；PCA 能把 1000 维特征压到 2 维画散点图；GAN（生成对抗网络）能从随机噪声生成逼真图像。注意一个细节——LLM 训练用的"自监督学习"是无监督学习的一个特例：标签不是人工标，而是从文本里天然派生出来的（下一句就是上一句的标签）。

**强化学习（Reinforcement Learning, RL）** 的设定不一样：有一个 Agent（智能体）在环境里不断做动作，环境会反馈奖励。Agent 学的是"在什么状态下做什么动作能拿最多奖励"。AlphaGo 击败世界冠军靠的就是 RL。RLHF（基于人类反馈的强化学习）也是大模型对齐的关键技术，参见 Ch5。

## 1.4 深度学习为什么崛起

深度学习的概念从 1980 年代就有，但真正大爆发是 2012 年之后的事。为什么等了 30 年才"突然"起飞？业内通常归结为四个驱动力同时到位：**算力、数据、算法、任务**。

**算力（Compute）** 方面，NVIDIA 在 2007 年开放 CUDA，让 GPU 不再只是游戏显卡，也能跑通用并行计算。神经网络训练本质是大规模矩阵乘法，正是 GPU 最擅长的。2016 年 Google 推出 TPU、之后各厂陆续推出 ASIC 加速卡，单个训练任务的算力提升了几倍量级。**数据（Data）** 方面，互联网与移动互联网产生了海量文本（Common Crawl 现已抓取到千亿级网页）和图像（Instagram 每天上传上亿张图）。这些数据天然带着"答案"——下一句就是上一句的答案。
...
**任务 / 评测（Benchmarks）** 像 ImageNet（1400 万张标注图像、1000 个类别）这样的公开基准，给了研究者一个明确的"考试卷"。谁家的模型分数高，论文就能发顶会、模型就能被工业采用。这种"明确目标 + 公开榜单"的组合极大加速了进步。
...
我们想强调一个常被误解的点：**LLM 不等于 AGI（通用人工智能，Artificial General Intelligence）**。它是一个"狭义但极强的通用语言接口"——你给它任何用自然语言描述的任务，它大概率能给出像样的回答；但它会一本正经地胡说（幻觉）、不知道今天几号（缺实时性）、不会真的操作你的电脑（缺工具）、也记不住三天前聊过什么（缺记忆）。
...
**Ch1 的定位**：全书入门起点，不预设任何 AI 背景。读者只要有任意一门编程语言基础（Python 优先）就能跟上。读完 Ch1，你应该能在脑子里画出一张清晰的 AI 全景图——下一章我们聚焦到"神经网络"这一格。
