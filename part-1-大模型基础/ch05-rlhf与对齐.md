...
左半边是训练（SFT → RM → PPO/DPO），右半边是部署后的循环（红队反哺训练）。下面 10 节展开。

## 5.1 为什么要对齐

经过 Ch4 的预训练和 SFT，模型已经"会接话"、大致"听得懂指令"，但还不是合格助手——会**答得出，不一定答得对**：可能答非所问、说胡话、甚至输出有害内容。
...
SFT 后的模型 MMLU 能打 80 分，但遇到"教我怎样制造炸弹"的诱导，照样可能"乐于助人"地给出步骤。**能力是"会不会"，对齐是"该不该"**。

## 5.2 RLHF 三步

**RLHF（Reinforcement Learning from Human Feedback，人类反馈强化学习）** 自 2022 年 InstructGPT / ChatGPT 后被广泛采纳。核心思想：**人难写完美的"奖励函数"，但能分清两个回答谁更好**。

**第一步：SFT（Supervised Fine-Tuning）**——用 `(指令, 回答)` 对继续训练 base 模型（见 Ch4）。

**第二步：奖励模型（RM）训练**——让标注员对同一问题的多个回答两两比较，用偏好对**打分器**做监督训练，输出标量分数。

**第三步：PPO 强化学习**——把 SFT 模型当作**策略（policy）**，用 RM 的分数当奖励信号，让模型通过试错学出"高分回答"。同时用 KL 惩罚把它拉回 SFT 附近，避免刷分胡说。

### 教小孩的类比

SFT = 看教科书；RM = 训练"考官"；PPO = 在模拟环境里按"考官"评分反复练习。
...
奖励模型是 RLHF 的"灵魂"。RM 的架构通常**和 SFT 模型一致**，但**最后一层换成回归头**——最后的隐藏状态过一个线性层，输出标量分数。
...
RM 学会后会给 A 打高分、B 打低分。它**不需要知道"为什么 A 好"，只要把好回答打得比坏回答更高**。InstructGPT 用约 3.3 万条偏好对，就是 RM 训练的全部数据。

### Bradley-Terry 公式

偏好对会被转成两两排序目标，常用 **Bradley-Terry 模型**——假设"A 比 B 好"的概率为：

$$P(A \succ B) = \sigma(r_\theta(A) - r_\theta(B))$$

其中 $r_\theta$ 是 RM 打分，$\sigma$ 是 sigmoid。训练目标：**让好回答分数减坏回答分数越大越好**。

### 工程上的两个常见坑

- **位置偏差**：标注员倾向选"上面的"——收集时要随机化顺序；
- **长度偏差**：标注员倾向选"更长的"——RM 也会学到它。

## 5.4 PPO 在 LLM 中的应用

**PPO（Proximal Policy Optimization）** 是 OpenAI 2017 年提出的强化学习算法，被 RLHF 借来更新 LLM——在 LLM 场景下涉及四个模型协同工作。

### 学生考试的类比

- **策略模型（Policy）** = 考试的学生（要训练）
- **参考模型（Reference）** = 手里的"标准答案"（冻结）
- **奖励模型（Reward）** = 阅卷老师（冻结）
- **价值模型（Value / Critic）** = 自我评估（要训练）
...
为什么需要参考模型？因为学生可能为拿高分"完全偏离标准答案"——输出怪字符、复读机式回答骗过 RM。这就是 **reward hacking**——奖励学习的头号敌人。**KL 散度惩罚**就是答案：策略跑得越远，惩罚越大。**参考模型是"安全绳"**。

### 现实痛点

PPO 是**显存怪兽**——四个模型同时在显存里，70B 训练要 80~160 张 H100。开源社区因此发展出**省显存变体**——LoRA-PPO、ReMax、GRPO，5.8 节展开。

## 5.5 DPO

**DPO（Direct Preference Optimization, Rafailov et al., 2023）** 是个优雅替代：**完全跳过奖励模型**，直接从偏好对学习策略。

### 直觉：只做考卷对照，不请老师

传统 RLHF 像学生做完题还得等老师批改；DPO 直接用"考卷"（偏好对）对照着学——模型自己琢磨好答案为什么好，再调参数。
...
**对比**：DPO 省掉 RM——少一个模型、一段训练、一堆显存。

### DPO 的核心观察

RLHF 的最优策略有闭式解——$\pi^*(y|x) \propto \pi_{\text{ref}}(y|x) \exp(r(x,y)/\beta)$。反过来：**知道最优策略的形状，就能直接从偏好对拟合**。
...
**三个优点**：极简（两个模型 + 偏好对）、稳定（无 reward hacking）、好用（2024 年后大多数开源 LLM 首选）。**坑**：对超参 $\beta$ 敏感、分布外泛化不稳。**记法**：DPO 与 RLHF 数学等价，工程上简单得多。

## 5.6 其他对齐方法

RLHF / DPO 不是唯一答案。下面**只深讲一个**，其余六个一句话定位。

### Constitutional AI：宪法法庭

**类比**：CAI 像"宪法法庭"。模型拿一本"宪法小册子"（一组原则）**自己评判、自行修改回答**；人类只负责写宪法。
...
**优点**：大幅减少人类标注；原则可解释、可审计。Anthropic 在 [Claude 新宪法（2026.01 发布）](https://www.anthropic.com/news/claude-new-constitution/) 中也纳入了对智能体行动场景的约束条款——CAI 正从"对话对齐"扩展到"行动对齐"。
...
对齐的另一面是**安全**——不仅要"答得好"，还要"不答坏"，这部分主要由**红队（red teaming）**承担。

**类比**：红队像专找漏洞的"白帽黑客"。

**常见攻击类别**（五类）：**越狱**、**提示注入**、**隐私提取**、**偏见触发**、**幻觉诱导**。

**工作流程**：攻击 prompt 库 → 扫模型 → 分析失败 → 补强 → 迭代。**自动化红队**用"攻击者 LLM"自动生成对抗 prompt，比纯人工高效数个量级。

为什么红队算对齐的一部分？**红队找到的失败案例会反哺回训练数据**——这是**对齐数据飞轮**的关键一环。

最后，**过度对齐会损害有用性**（safety-helpfulness tradeoff），只能按部署场景调节。

## 5.8 推理模型与思维链

2024 年下半年起，LLM 出现了"**推理模型**"新赛道——[OpenAI o1](https://openai.com/index/learning-to-reason-with-llms/)（2024.09）、DeepSeek-R1（2025.01）、Claude 3.7 Sonnet Extended Thinking（2025.02）等。核心思想：

> **"思考更久，答得更好"** ——用更多推理时算力，换取更高回答质量。

**直觉**：传统 LLM 像**抢答选手**——立刻回答；推理模型像**草稿选手**——先"长考"再给答案。

**思维链（CoT）** 是基础：给几个"思考 → 答案"示例，模型就能生成中间步骤。推理模型把 CoT 从"prompt 技巧"升级成"训练范式"——**让模型在训练时就学会"先想后答"**：
...
**DeepSeek-R1** 的核心算法叫 **GRPO**——PPO 的"简化版"：不训价值模型；对同一 prompt 采样一组回答；用组内**相对分数**作优势估计。四模型砍到两模型，显存减半。

### PRM：给每步打分

**Process Reward Model（PRM）**——传统 RM 只给最终答案打分，PRM 给每一步推理打分。
...
前四章讲的都是文本 LLM，但现实信息远不止文字：图、视频、表格、音频……**多模态 LLM（Multimodal LLM, MLLM）** 把这些模态统一处理。

**类比**：多模态像"翻译不同语言"——文本是普通话、图像是粤语、音频是英语，Transformer 是把它们都译成"通用语"（token）的超级翻译官。
...
**路线一：CLIP-style（LLaVA 路线）**——单独训视觉编码器 + 语言模型，用投影层把视觉特征映射到 LLM。优：模块化、训练便宜。代表：LLaVA、Qwen-VL、InternVL。

**路线二：Native Multimodal（GPT-4V / Gemini 路线）**——从一开始就混训多模态 token。优：端到端；劣：训练贵。代表：GPT-4V、Gemini、Claude。

**特殊挑战**：跨模态语义对齐、细粒度理解、长视频、多模态幻觉。**简单记法：** CLIP-style 模块化拼接，Native 端到端融合。

## 本章小结

Part 1 的旅程告一段落，回顾走过的路：
...
Part 1 讲"**原理**"，Part 2 讲"**家族演进**"——主流 LLM 各自走了什么变体、什么设计选择。Part 2 会带你走 GPT / LLaMA / DeepSeek / Qwen / Claude / Gemini 等系列，展开 Ch3 的 3.10 节提到的 MoE / RMSNorm / SwiGLU / GQA 等现代组件，以及 5.6 节提到的各种对齐方法的演进。准备好了吗？我们出发。
