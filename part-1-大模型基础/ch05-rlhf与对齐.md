...
RLHF 的最优策略有闭式解——$\pi^*(y|x) \propto \pi_{\text{ref}}(y|x) \exp(r(x,y)/\beta)$。反过来：**知道最优策略的形状，就能不用奖励、直接从偏好对拟合**。
...
**工作流程**：构造攻击 prompt 库 → 扫模型 → 分析失败 → 补强 → 迭代。**自动化红队**用"攻击者 LLM"自动生成对抗 prompt，比纯人工高效数个量级。
